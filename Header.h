...
void KL_partition(int indx, float **graph, int n, int *core_id,
                  int *final_partition_core);
float flip(float **G, int *final_partition_core, int k, int t, int nodes,
           int local);
void iterative_improvement(float **G, int *final_partition_core, int nodes,
                           int local);
...
void KL(int indx, float **graph, int *core_id, int n, int **partition) {
  ...
  float Ia, Ib, Ea, Eb, Pow_a, Pow_b, *Da, *Db, gain, temp, *Gain_k,
      *Sum_Gain_k, max;
    ...
    for (index2 = index1 + 1; index2 < end; index2++) {
      if (G[map[index1]][map[index2]] != MAX &&
        ...
        if (G[map[index1]][map[index2]] == G[map[index2]][map[index1]]) {
          cost = cost +
                 (float)Hops(index1, index2, n) * G[map[index1]][map[index2]];
        }
  ...
  for (index1 = 0; index1 < n; index1++) {
    for (index2 = index1 + 1; index2 < n; index2++) {
      if (G[map[index1]][map[index2]] != MAX &&
          G[map[index1]][map[index2]] != 0) {
        if (G[map[index1]][map[index2]] == G[map[index2]][map[index1]]) {
          cost = cost + (float)(Hops(index1, index2, n) + 1) * 2 *
          ...
          cost =
              cost +
              (float)(Hops(index1, index2, n) + 1) * 2 *
